...
import { columnsReportTraccarResumen } from '../../utils/ExportColumns';

const columns = [
  { id: 'deviceId', label: 'VEHICULO', minWidth: 170, key: 1 },
  { id: 'startTime', label: 'HORA DE INICIO', minWidth: 170, key: 2 },
  { id: 'distance', label: 'DISTANCIA', minWidth: 170, key: 3 },
  { id: 'averageSpeed', label: 'VELOCIDAD PROMEDIO', minWidth: 170, key: 4 },
  { id: 'startOdometer', label: 'ODOMETRO INICIAL', minWidth: 170, key: 5 },
  { id: 'endOdometer', label: 'ODOMETRO FINAL', minWidth: 170, key: 6 },
  { id: 'maxSpeed', label: 'VELOCIDAD MAXIMA', minWidth: 170, key: 7 },
  { id: 'engineHours', label: 'HORAS DE MOTOR', minWidth: 170, key: 8 },
  { id: 'engineHoursMoving', label: 'HORAS DE MOTOR EN MARCHA', minWidth: 170, key: 9 },
  { id: 'engineHoursIdle', label: 'HORAS DE MOTOR PARADO', minWidth: 170, key: 10 },
  { id: 'spentFuel', label: 'COMBUSTIBLE GASTADO', minWidth: 170, key: 11 }
];

export default function ResumenPage() {
  const [vehiculos, setVehiculos] = useState([]);
  const [grupos, setGrupos] = useState([]);
  const [searchFilter, setSearchFilter] = useState({ date_filter: 'today', daily: 'false' });
  ...
  async function SearchFilter() {
    ...
    try {
      const result = await getTraccar(
        '/reports/summary' + construirUrl({ ...searchFilter, from: from, to: to, from_date: undefined, to_date: undefined })
      );
      if (result.status === 200) {
  ...
  function CleanFilter() {
    setData([]);
    setSearchFilter({ date_filter: 'today', daily: 'false', from_date: '', to_date: '' });
  }
  ...
  const ExportFilter = async () => {
    try {
      ...
      exportToExcel(dataParse, columnsReportTraccarResumen, 'Resumen');
    } catch (error) {
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} md={4}>
              ...
              <Autocomplete
                ...
                getOptionLabel={(option) => option.name}
                onChange={(event, newValue) => {
                  setSearchFilter({ ...searchFilter, groupId: newValue ? newValue.id : '' });
                }}
                value={grupos.find((groupId) => groupId.id === searchFilter.groupId) || null}
                renderInput={(params) => <TextField {...params} />}
              />
            </Grid>
            <Grid item xs={6} md={4}>
            ...
            <Grid item xs={6} md={4}>
              ...
              <TextField
                ...
                size="small"
                onChange={handleSearchChange}
                value={searchFilter.daily}
              >
                <MenuItem value="false">Resumen</MenuItem>
                <MenuItem value="true">Resumen Diario</MenuItem>
              </TextField>
            </Grid>
          </Grid>
        </Grid>
