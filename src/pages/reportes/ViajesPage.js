...
const MostarCalle = ({ row }) => {
  ...
  const handleClick = async () => {
    try {
      const response = await getTraccar(`server/geocode?latitude=${row.endLat}&longitude=${row.endLon}`);
      if (response) {
...
export default function ViajesPage() {
  ...
  const SearchFilter = async () => {
    if (!searchFilter.deviceId) {
      notificationSwal('error', 'Debe seleccionar un dispositivo');
      return;
    }
